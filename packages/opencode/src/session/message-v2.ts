...
export const toModelMessagesEffect = Effect.fnUntraced(function* (
  ...
  for (const msg of input) {
    ...
    if (msg.info.role === "assistant") {
      ...
      // Track whether we've seen a tool part in the current step. If text or
      // reasoning appears after a tool without an intervening step-start, inject
      // a synthetic boundary so provider replays don't interleave tool_use with
      // later assistant content in one block.
      let sawTool = false
      for (const part of msg.parts) {
        if ((part.type === "text" || part.type === "reasoning") && sawTool) {
